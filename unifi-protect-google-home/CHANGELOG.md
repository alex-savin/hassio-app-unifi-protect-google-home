...
- **Side-panel control + hot-apply.** The Bridge card in the ingress page now has a dropdown plus an *Apply* button. Saving POSTs to a new `/api/log-settings` endpoint that persists `bridge.ws_event_log` via the Supervisor options API and hot-applies it to the running reconciler through a new `setup.WSLogApplier` interface — no add-on restart needed; the change takes effect on the very next websocket frame.

## 0.3.26

- **Per-camera exposure control.** New `bridge.exposed_cameras` add-on option (list of camera IDs) lets the user pick exactly which Protect cameras get advertised to Google Home. Hidden cameras are dropped from the SYNC device list, return `online=false` in QUERY, and `GetCameraStream` EXECUTE is refused (`deviceOffline`) — so their video never leaves the LAN even if Google still has a cached device entry from before the list was trimmed. The reconciler also stops firing HomeGraph `ReportState` / `RequestSync` for hidden cameras, keeping ReportState submissions valid (HomeGraph rejects entries for IDs it never received via SYNC). An empty list keeps the historical "all cameras allowed" default, so existing installs upgrade transparently.
- **Side-panel UI for camera exposure.** The Cameras card in the ingress page now renders a checkbox per camera plus Save / Select all / Select none controls. Saving POSTs to a new `/api/cameras` endpoint that (a) persists `bridge.exposed_cameras` via the Supervisor options API and (b) hot-applies the change to the running bridge through a new `setup.CameraAllowlistApplier` interface — no add-on restart needed. When HomeGraph is wired the applier also fires `RequestSync` so Google re-pulls the device list immediately.
- **`CameraInfo.exposed` field** in the `/api/status` JSON so the UI can show a per-camera `exposed / hidden` pill alongside the existing `online / offline` pill and the doorbell flag.

## 0.3.25

- **Side-panel page is now a live status dashboard.** The ingress UI used to be a setup-only wizard. It now fetches `/api/status` on load and renders the running bridge's state: UniFi controller (host, NVR MAC, Protect version, connected/disconnected), the live camera list (name, model, online/offline, doorbell flag), Google linkage (HomeGraph enabled / service-account loaded / OAuth client configured / `project_id` mismatch warning), and bridge wiring (public base URL, listen address, agent user ID, persisted SYNC fingerprint). The reconfigure-credentials wizard is preserved behind a collapsible section that opens automatically when the bridge is in setup-only mode or UniFi is disconnected. A new `setup.StatusProvider` interface is wired from `cmd/bridge/main.go` so the page reflects the same in-memory state the fulfillment handler serves to Google.
- **Dependencies bumped to latest** within compatible majors: `pion/interceptor` 0.1.45, `pion/turn/v5` 5.0.4, `golang.org/x/{crypto,net,sys,time}` to their May 2026 releases. `gortsplib` held at v4.16.2 — upstream tagged v4.16.3 as a deprecation stub and moved to v5.x, which is a breaking refactor of the format/description packages we use across `internal/{rtsp,hls,mp4}` and will be migrated separately.
- **Repo cleanup.** Removed seven dead files (`Dockerfile`, `config.yaml`, `go.mod`, and four `internal/*.go`) at the workspace root that were stale duplicates of the add-on's real sources under `unifi-protect-google-home/`. CI was already pointing at the subdir; nothing shipped to users changes.
...
- **Operational note: `action.devices.DISCONNECT` means manual re-link.** When the bridge logs `ghome intent: action.devices.DISCONNECT` after a long quiet period, the Google account has been unlinked from the test/release project — typically because a previous OAuth client was revoked or the integration was removed in the Home app. Google will not send any further SYNC/QUERY/EXECUTE intents until the user re-adds the test action in the Home app. v0.3.22 does not attempt to recover automatically; the handler returns the spec-mandated empty payload and exits.
